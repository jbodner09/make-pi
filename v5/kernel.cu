...
order to get maximum performance. For example, on my own personal machine,
which has a GeForce GT 650M discrete graphics card, there are now only 
12 streaming multiprocessors (SM's), with 32 cores each, for a total of
384 cores. Thus, 384 threads will be created, with each thread performing
...
Compilation on my own machine actually makes OSC's steps look like child's
play.  The best way to do this is to download Visual Studio. Then, go to
http://developer.nvidia.com/nsight-visual-studio-edition-downloads
and follow the steps to install the proper graphics driver, then the CUDA 
toolkit, then the Nsight Visual Studio Plugin. So, the project you create
will be a new CUDA project, which will link to the proper headers. Compile
it, then go to NSIGHT in the menu > Start CUDA Debugging. Eventually, the 
proper result will pop up in the console window that opens. Note that if
you want to change the number of iterations/precision, you'll need to 
edit the VS project settings under PROJECT > "Name" Properties... and
then under Configuration Properties > Debugging, add the values you want
to the Command Arguments field, since the default is to build the 
project for Debug, not Release. Also, the sleep function is added just
because the debug console window disappears in Visual Studio. Grr. 

If you want to run this separately... you can't. I tried to compile this
from the command line. It worked, even though I had to add extra system
variables for NVCC to Window's Environment Variables, and then pass the
path to VS's C compiler to NVCC when I ran it. Running it, however, did
not. It crashed the graphics driver and returned that an error occurred
upon trying to copy the results back from the GPU. Even debugging using
DEBUG > Start Debugging in VS crashed it that way.  Only debugging via
NSIGHT actually worked. But, whatever, we got results. Remember that 
this solution uses dynamic memory allocation on the device, so only 
CUDA 2.0+ devices will run this code. 
*/

// Includes. Optimum values for my computer are:
// NUM_BLOCKS 12
